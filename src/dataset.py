...
def semi_supervised_preprocess(dataset, num_of_client, split_type, weights, alpha, min_size, class_per_client,
    ...
    _, train_data,_ = data_simulation(train_data['x'], train_data['y'], num_of_client, split_type, weights, alpha, min_size, class_per_client)
...
def get_semi_supervised_dataset(dataset, num_of_client, split_type, class_per_client, label_ratio=0.01, image_size=32,
                                gaussian=False):
    train_data, test_data, labeled_data = semi_supervised_preprocess(dataset, num_of_client, split_type, None, 0.5, 10, class_per_client, label_ratio)
